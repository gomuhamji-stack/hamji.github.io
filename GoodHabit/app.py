...
def init_db():
    ...
    # 월별 목표 테이블 추가
    conn.execute('''CREATE TABLE IF NOT EXISTS goals (year_month TEXT PRIMARY KEY, content TEXT)''')
    
    # 초기 설정값 (기존 데이터가 없으면 삽입)
    conn.execute("INSERT OR IGNORE INTO settings (key, value) VALUES ('h1_name', 'RUNNING')")
    conn.execute("INSERT OR IGNORE INTO settings (key, value) VALUES ('h2_name', 'COFFEE')")
    conn.execute("INSERT OR IGNORE INTO settings (key, value) VALUES ('h3_name', 'STORYWRTHY')")
    conn.commit()
    conn.close()
...
def get_all_data(year_month):
    ...
    goal = conn.execute("SELECT content FROM goals WHERE year_month = ?", (year_month,)).fetchone()
    conn.close()
    return jsonify({
...
def save_goal():
    data = request.json
    conn = get_db_connection()
    conn.execute("INSERT INTO goals (year_month, content) VALUES (?, ?) ON CONFLICT(year_month) DO UPDATE SET content=excluded.content", 
